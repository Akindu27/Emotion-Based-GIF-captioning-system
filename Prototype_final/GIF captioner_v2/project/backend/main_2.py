...
def detect_content_type(frame: Image.Image) -> tuple[str, float]:
    ...
    Strategy: Use ONLY the most reliable indicators
    """
    try:
        ...
        # Count unique colors (most reliable for cartoons vs real)
        ...
        # Face detection (strong real-world indicator)
        face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        faces = face_cascade.detectMultiScale(gray, 1.1, 4)
        has_realistic_face = len(faces) > 0
        
        # Debug logging
        print(f"🔍 Content Detection - Color: {color_ratio:.3f}, Sat: {saturation:.3f}, Face: {has_realistic_face}")
        
        # RULE 1: Face detected = definitely real-world
        if has_realistic_face:
            print("   → Real-world (face detected)")
            return "real_world", 0.95
        
        # RULE 2: Very low color diversity + high saturation = cartoon
        # This catches most cartoons reliably
        if color_ratio < 0.01 and saturation > 0.3:
            print("   → Cartoon (very low colors + saturated)")
            return "cartoon", 0.9
        
        # RULE 3: Extremely low color diversity = likely cartoon
        if color_ratio < 0.005:
            print("   → Cartoon (extremely low colors)")
            return "cartoon", 0.8
        
        # RULE 4: Low colors + moderate/high saturation = probably cartoon
        if color_ratio < 0.02 and saturation > 0.4:
            print("   → Cartoon (low colors + saturated)")
            return "cartoon", 0.7
        
        # RULE 5: Everything else = real-world (to avoid false positives)
        print("   → Real-world (default)")
        return "real_world", 0.6
            
    except Exception as e:
        print(f"❌ Error detecting content type: {e}")
        return "real_world", 0.5  # Default to real-world on error
