...
def run_experiment(
    ...
    print("\n⚖️  Computing MODERATE class weights...")
    ...
    # CAP EXTREME WEIGHTS - this is the key fix!
    # Previous attempt: weights up to 5.4× caused model to forget majority class
    # Solution: Cap at 2.5× maximum
    MAX_WEIGHT = 2.5
    MIN_WEIGHT = 0.5
    
    # Normalize to [0.5, 2.5] range
    weight_min = class_weights.min()
    weight_max = class_weights.max()
    class_weights_normalized = (class_weights - weight_min) / (weight_max - weight_min)  # [0, 1]
    class_weights_capped = MIN_WEIGHT + class_weights_normalized * (MAX_WEIGHT - MIN_WEIGHT)  # [0.5, 2.5]
    
    class_weights = torch.FloatTensor(class_weights_capped).to(device)
    
    print(f"   MODERATE class weights computed (capped to {MIN_WEIGHT}-{MAX_WEIGHT}×):")
    emotion_names = list(emotion_to_idx.keys())
    for idx, weight in enumerate(class_weights):
        print(f"      {emotion_names[idx]:20s}: {weight:.3f}")
    
    # Training setup (WITH MODERATE CLASS WEIGHTS!)
